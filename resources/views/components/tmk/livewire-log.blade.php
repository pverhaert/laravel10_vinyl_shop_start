@php
    ...
    $inView = false;                // show the result in the view in json format (true)
    /* end config ----------------- */
    ...
    $output = [];
    $prefixes = ['__', 'app', 'errors', 'attributes', 'slot', 'output', 'prefixes', 'show', 'onlyDebugMode', 'sidebar', 'console', 'consoleJson', 'inView'];
...
<div>
    ...
    @if($inView)
        @foreach(array_reverse($output) as $key => $value)
            <pre class="bg-red-300 my-4 p-2 text-sm">@json($genres, JSON_PRETTY_PRINT)</pre>
        @endforeach
    @endif
</div>
